...
pub fn add_ingame_menu(mut commands: Commands, fonts: Res<Fonts>) {
    commands.spawn(ingame_menu()).with_children(|parent| {
        parent.spawn(ingame_menu_button()).insert(ButtonAction::Resume).with_children(|parent| {
        ...
        parent.spawn(ingame_menu_button()).insert(ButtonAction::EditUi).with_children(|parent| {
            parent.spawn(ingame_menu_button_text("Hud Editor".to_string(), &fonts));
        });
        parent.spawn(ingame_menu_button()).insert(ButtonAction::Lobby).with_children(|parent| {
            parent.spawn(ingame_menu_button_text("Return to Lobby".to_string(), &fonts));
        });
        parent.spawn(ingame_menu_button()).insert(ButtonAction::Exit).with_children(|parent| {
            parent.spawn(ingame_menu_button_text("Exit Game".to_string(), &fonts));
        });
    });
}

pub fn toggle_ingame_menu(mut ingame_menu: Query<&mut Visibility, With<InGameMenuUi>>, state: Res<State<InGameMenu>>) {
...
pub fn state_ingame_menu(kb: Res<Input<KeyCode>>, mut next_state: ResMut<NextState<InGameMenu>>, state: Res<State<InGameMenu>>) {
