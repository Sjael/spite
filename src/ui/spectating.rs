...
pub fn update_health(
    ...
    let Ok(attributes) = query.get(spectating.0) else {
        return;
    };
...
pub fn update_cc_bar(
    ...
    let Ok(mut style) = cc_bar_fill.get_single_mut() else {
        return;
    };
...
pub fn update_buff_stacks(
    ...
    for stack_change in stack_events.iter() {
        ...
        for (buff_ui_entity, buff_id, mut despawn_timer) in buff_holders.iter_mut() {
            ...
            break; // return cus we found the buff, dont return cus we want to go to next event
        }
    }
}
...
pub fn update_objective_health(
    ...
    let Some(focused_entity) = focused_health_entity.0 else {
        return;
    };
    let Ok(mut bar) = bar_query.get_single_mut() else {
        return;
    };
    let Ok(attributes) = query.get(focused_entity) else {
        return;
    };
    let current = *attributes.get(&Stat::Health.as_tag()).unwrap_or(&0.0);
    let max = *attributes.get(&Stat::HealthMax.as_tag()).unwrap_or(&100.0);
